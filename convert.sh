...
while IFS= read -r line; do
  ...
  FONTS_SEMIBOLD=$(find "$DIR" -type f | grep -Ei '\.(ttf|otf|woff2?|eot)$' | grep -i ' semi bold\.')
...
@font-face {
  font-family: '$NAME';
  src: url('fonts/$NAME/$NAME Semi Bold.woff2') format('woff2'),
       url('fonts/$NAME/$NAME Semi Bold.woff') format('woff'),
       url('fonts/$NAME/$NAME Semi Bold.ttf') format('truetype');
  font-weight: 600;
  font-style: normal;
}
EOF
